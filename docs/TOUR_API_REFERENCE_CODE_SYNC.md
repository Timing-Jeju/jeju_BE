...
JSON은 envelope를 포함한 모든 object에서 중복 key를 거부합니다. 법정동·관광 분류의 known code/name field는 JSON string이면서 trim 후 non-blank여야 하며 number, boolean, null, object, array coercion을 허용하지 않습니다. additive unknown scalar field는 이 known field 타입 정책의 대상이 아니지만 중복 key 정책은 동일하게 적용합니다. XML은 namespace 없는 exact envelope를 사용하며 header와 item의 모든 known/unknown direct field 이름을 각각 한 번만 허용합니다. XML scalar는 text와 CDATA node만 이어 붙이며 element, comment, processing instruction 같은 child node가 섞인 mixed content는 거부합니다. 두 형식 모두 duplicate와 nested scalar spoof를 전체 응답 실패로 처리합니다.
