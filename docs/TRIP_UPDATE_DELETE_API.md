...
- OpenAPI inventory: 23 operations, `validate_openapi_frontend_readiness.py --mode 23`
...
- completed/cancelled/failed: 실행 중 run이 없으면 삭제 허용
- trip aggregate 자식은 FK cascade로 제거
- 공유 `tour_places`, `data_import_runs`, `user_profiles`, `auth.users`는 보존

## Database

`20260906000000_trip_update_delete_contract.sql`은 다음을 적용한다.
