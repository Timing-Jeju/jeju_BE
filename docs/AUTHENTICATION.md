...
필수 API의 `Authorization` header가 없으면 `401 AUTHENTICATION_REQUIRED`, header를 제공했지만 형식·서명·claim·만료·key 검증이 실패하면 `401 INVALID_ACCESS_TOKEN`입니다. 선택 인증 API는 header가 없을 때 익명으로 처리하고 제공된 인증 정보가 실패하면 `INVALID_ACCESS_TOKEN`을 반환합니다. 빈 값, 잘못된 scheme, 빈 Bearer, 중복 header도 제공된 인증 정보 실패로 닫힙니다. 인증된 사용자의 접근 거부와 CORS 거부는 `403 AUTH_ACCESS_DENIED`입니다. 알려진 unknown `kid`와 네트워크 접근 실패 또는 JWKS endpoint의 HTTP 5xx 같은 원격 가용성 장애는 token을 노출하지 않고 `INVALID_ACCESS_TOKEN`으로 종료합니다. HTTP 200 응답의 malformed/invalid JWKS payload처럼 provider protocol을 신뢰할 수 없는 경우와 예상하지 못한 decoder/provider 내부 장애는 401로 숨기지 않고 `500 AUTH_INTERNAL_ERROR`로 분류합니다. 모든 보안 오류는 `application/problem+json`이며 `type`, `title`, `status`, `detail`, `instance`, `code`, 요청 단위 `traceId`, `fieldErrors`를 반환합니다. validation이 아니면 `fieldErrors=[]`이고 `message` 필드는 사용하지 않습니다. 분류는 endpoint path나 예외 message가 아니라 Authorization header 존재 여부와 원인 타입을 사용합니다. `instance`는 `X-Trace-Id` 헤더와 body의 `traceId`에 쓴 32자리 소문자 hex 값을 포함한 occurrence URI `urn:timing-jeju:problem:<traceId>`이며 raw path를 반사하지 않습니다. Spring Boot가 관리하는 Jackson 3 mapper bean과 공통 writer로 직렬화하며 token, JWT payload, JWKS body, URL query, 예외 message와 개인정보를 응답이나 애플리케이션 로그에 기록하지 않습니다.
...
Supabase 공식 JWKS 경로는 `/auth/v1/.well-known/jwks.json`입니다. Spring의 Nimbus decoder가 공개키 조회와 캐시를 담당하며 애플리케이션 시작 시에는 JWKS 네트워크 호출을 요구하지 않습니다. 기존 `kid`를 최초 검증한 뒤 JWKS가 old→old+new로 바뀌면 unknown `kid`에서 한 번 재조회해 새 key를 검증합니다. 한 번 조회한 key는 캐시에 남아 있으므로 JWKS endpoint의 일시 장애 중에도 같은 `kid`의 기존 token을 검증할 수 있습니다. 아직 조회하지 않은 `kid`나 cache 밖의 key는 장애 중 `401 INVALID_ACCESS_TOKEN`으로 거부합니다. 반면 endpoint가 HTTP 200으로 응답하더라도 body가 유효한 JWKS가 아니면 provider protocol fault로 보아 안전한 500을 반환합니다. 테스트 서버는 `Cache-Control: max-age=300`을 사용해 실제 Nimbus cache/unknown-`kid` 재조회 동작을 검증합니다.
...
회원 탈퇴 기능은 `APP_ACCOUNT_DELETION_ENABLED=true`일 때만 API, pending 접근 차단, scheduler/worker가 함께 활성화됩니다. 기본값은 `false`이며 앱 심사 배포에서는 이 값을 반드시 `true`로 지정하고 `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `APP_ACCOUNT_DELETION_WORKER_ID`, `APP_ACCOUNT_DELETION_STATUS_TOKEN_ENCRYPTION_KEY`를 함께 주입해야 합니다. 필수 secret이나 URL 없이 활성화하면 서버가 fail-fast합니다.
...
탈퇴 요청이 `queued` 또는 `running`이면 Spring 공통 authorization 경계가 해당 JWT의 보호 API 접근을 차단합니다. 탈퇴 요청의 멱등 `DELETE /api/v1/me` replay와 status-token 기반 GET만 계속 허용합니다. worker step의 기존 `SESSIONS_REVOKED` 이름은 DB resume 호환을 위해 유지하지만 의미는 이 Spring deny gate의 활성 상태 검증입니다. Supabase에는 subject만으로 모든 세션을 logout하는 Admin endpoint가 없으므로 그런 호출을 가장하지 않습니다. 최종 `DELETE /auth/v1/admin/users/{userId}`가 refresh token과 Auth session을 cascade 제거하며, 이미 발급된 JWT는 만료 시점까지 유효할 수 있으므로 Spring deny gate를 Storage·앱 데이터·Auth 삭제보다 먼저 검증합니다.
