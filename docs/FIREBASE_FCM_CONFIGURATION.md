...
launcher는 portable Python `os.stat` preflight로 absolute path, regular/non-symlink, 현재 UID/GID와 owner-only permission을 먼저 확인합니다. 이어서 exact `docker compose version --short` 결과가 strict `v?MAJOR.MINOR.PATCH` 또는 `v?MAJOR.MINOR.PATCH-desktop.<positive integer>`이며 numeric core가 2.24.4 이상인지 확인합니다. beta·rc·build metadata와 임의 suffix는 거부합니다. 두 preflight는 credential 내용, 경로 또는 명령의 raw 오류 출력을 노출하지 않으며 조건이 하나라도 다르면 기존 API를 중단하지 않습니다.
...
FCM API의 host binding은 caller의 `API_PORT`와 무관하게 loopback 전용 `127.0.0.1:18083`으로 고정됩니다. 기본 API의 `0.0.0.0:8080` binding을 상속하지 않습니다.
...
FCM opt-in API와 credential staging만 정리할 때는 인자 없이 고정 cleanup launcher를 사용합니다.
