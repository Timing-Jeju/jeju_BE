...
교체/clear가 커밋되면 이전 immutable object의 exact key+Storage ETag를 cleanup outbox에 기록한다. worker는 outbox claim 뒤 profile row를 잠그고 DB current reference가 그 key를 가리키지 않는지, owner/key/generation/Storage ETag가 enqueue 시점과 같은지 다시 확인한 경우에만 Storage API로 삭제한다. unique `(object_key, storage_etag)`와 claim/status/attempt/next-attempt가 중복 실행을 막고 새 generation과의 interleaving이 새 객체를 대상으로 바꾸지 못하게 한다. 첫 upload 후 확정 실패 orphan도 grace cutoff 이전이며 어떤 profile도 참조하지 않는 immutable generation에 한해 owner/key/generation/ETag를 재확인한다. 검증, metadata 조회, stale/version·멱등 충돌 실패는 profile을 변경하거나 cleanup을 enqueue하지 않는다. 탈퇴는 #106이 Auth 삭제 전에 canonical user prefix를 list하고 exact canonical-key 객체만 server Storage API로 제거한다.

Supabase `/object/info`의 공식 InfoRenderer 응답은 `id/name/version/bucket_id/size/content_type/etag/metadata/last_modified`를 사용하며 `owner_id`를 노출하지 않는다. 따라서 runtime owner 권위는 사용자 제공 metadata가 아니라 authenticated INSERT RLS의 `owner_id = auth.uid()`, exact canonical key, immutable generation(UPDATE/DELETE deny)과 현재 요청의 canonical subject/key 결합에서 도출한다. `metadata.generation`은 이 권위의 독립적인 일관성 신호일 뿐이다. 서버는 official info의 name/bucket/id/version/size/content-type/generation/strong ETag/time과 별도 HEAD ETag의 byte-exact 일치를 확인한다. 고정 provenance fixture는 `fixtures/contracts/profile-legal/supabase-storage-info.json`이다.

Orphan scan은 공식 Storage list v1의 `limit/offset`과 `sortBy: name asc`를 사용하고 app-owned private CAS cursor(owner/object offset, revision)를 tick 사이에 보존한다. 한 tick은 제한된 owner/profile/leaf 페이지만 읽으며 끝에서 wrap하여 앞쪽 삭제로 offset이 당겨져 건너뛴 객체도 다음 cycle에 재방문한다. experimental list-v2와 `storage` schema custom object/direct DML은 사용하지 않는다.
