...
현재 통합 브랜치는 #68의 27-operation artifact에 transport-event PUT/DELETE를 더한 exact 29-operation artifact를 기준으로 한다.
...
11. portable validator와 mutation test는 artifact 부재를 포함해 fail-closed다. #47 통합 브랜치는 새로 생성한 단일 29-operation artifact에서 `--mode 29` 검사를 통과해야 Codegen READY다. historical `--mode 27`은 transport-event를 allowlist 밖으로 거부한다. 기능별 문서나 fixture를 합쳐 만든 JSON은 완료 증거로 인정하지 않는다.
