...
> **현재 통합 공개 API 21개는 Codegen READY 검증 대상이다.** `openApiDocs` 뒤 portable frontend-readiness validator의 `--mode 21` 명령이 #182의 9개, #34/#44의 7개, #113의 4개와 #46의 여행 선호 endpoint 1개를 exact inventory로 고정하고 operationId, media type, header, schema/example 양방향 정합성, Problem Details, 비밀정보와 내부 경로를 fail-closed로 검사한다. historical `--mode 16`과 `--mode 20`은 이전 inventory를 그대로 보존한다.

이 문서는 2026-09-01 현재 구현이 끝난 공개 Spring API 21개 operation의 프론트엔드 인계본이다. 모든 예시는 공개 가능한 고정 fixture이며 token, provider secret, 실제 사용자 정보가 아니다. 서버가 받지 않는 필드와 문서에 없는 enum을 추가하지 않는다.

## 기준과 브랜치 준비 상태

| 상태 | 범위 | 권위 자료 |
|---|---|---|
| `develop` 사용 가능 | 기존 공개 API 20개 | `origin/develop` `ae3926ac6428c1d93cd57372fbafb8dd31d34544`의 Controller/OpenAPI와 canonical contract |
| **#46 구현 검증** | trip preferences 1 | `preferences-transport` 1.0.0과 `PUT /api/v1/trips/{tripId}/preferences`; transport event/place preference는 후속 구현 대기 |

현재 #46 브랜치의 생성 OpenAPI에는 기존 20개와 trip preferences 1개가 함께 존재한다. 다만 #46은 독립 Reviewer 승인과 병합 전이므로 배포된 `develop`에서 21번째 endpoint를 호출할 수 있다고 간주하지 않는다. 아직 미구현인 trip PATCH/DELETE, transport event, place preference는 포함하지 않는다.
...
통합된 21개 operation은 #182 customizer가 확장한 stable lowerCamelCase operationId를 제공한다. #113은 `pushDevicesUpdate`, `pushDevicesDelete`, `notificationPreferencesRead`, `notificationPreferencesUpdate`를, #46은 `tripPreferencesUpdate`를 추가한다. `_1` 같은 자동 suffix 또는 generic `list/read/create/update/delete`가 다시 나타나면 품질 게이트가 실패한다.
...
saved place PATCH와 trip preferences PUT은 `If-Match`가 필수다. 성공의 `ETag`를 client cache에 저장하고 다음 변경 요청에 그대로 보낸다. 누락·형식 오류는 `400 INVALID_REQUEST`다. stale 값은 saved place에서 `409 SAVED_PLACE_VERSION_CONFLICT`, trip preferences에서 `409 TRIP_VERSION_CONFLICT`다.
...
11. portable validator와 mutation test는 artifact 부재를 포함해 fail-closed다. #182의 9개, #34/#44의 7개, #113의 4개와 #46의 1개 endpoint는 네 선행 clean SHA를 조상으로 포함한 checkout에서 새로 생성한 단일 21-operation artifact에 나타나 `--mode 21` 검사를 통과해야 Codegen READY로 승격된다. `--mode 16`과 `--mode 20`은 각 historical inventory 밖 endpoint를 거부한다. 기능별 문서나 fixture를 합쳐 만든 JSON은 완료 증거로 인정하지 않는다.
