...
- [schedule revision run foundation](../../supabase/migrations/20260830000000_schedule_revision_run_foundation.sql)
...
### Issue #181 profile image future additive schema (#78 구현 소유)

이 문서는 migration을 실행하지 않고 #78이 추가할 형태만 고정한다. `user_profiles`에는 nullable `profile_image_object_key`, `profile_image_storage_etag`, `profile_image_source text not null`과 `profile_image_version bigint not null default 0`을 추가한다. version은 `0..9223372036854775807` 범위에서 성공한 non-replay mutation마다 정확히 1 증가하고 상한에서는 409로 실패한다. source가 `storage`이면 immutable `{lowercase-sub}/profile/{lowercase-generation-uuid}` key와 strong Storage ETag가 모두 필요하고, `provider` 또는 `none`이면 둘 다 null이어야 한다. 기존 row는 legacy provider URL이 있으면 `provider`, 없으면 `none`으로 backfill하고 key/ETag는 null, version은 0으로 둔다. Storage public/signed URL은 저장하지 않는다. core `GET /api/v1/me`와 companion GET은 모두 `storage > provider > none` projection을 사용한다.

`20260810000000_api_idempotency_registry.sql`의 기존 `api_idempotency_records`를 재정의하지 않고 그대로 재사용한다. PK는 `(owner_sub, http_method, normalized_path, idempotency_key)`이며 기존 `attempt_token`, `PROCESSING|COMPLETED`, `bytea response_headers/response_body`, lease/completed/expiry lifecycle을 바꾸지 않는다. profile-image request hash는 canonical body와 original If-Match를 묶고, 완료 snapshot의 status와 Content-Type/ETag header bytes 및 body bytes를 보존하여 replay가 stale CAS보다 먼저 exact 응답을 반환하게 한다.

`profile_image_cleanup_outbox`의 future exact fields는 `id`, `owner_user_id`, `object_key`, `storage_etag`, `source_profile_version`, `reason`, `status`, `claim_token`, `claimed_at`, `attempt_count`, `next_attempt_at`, `completed_at`, `created_at`이다. reason은 `replacement|clear|orphan|account_deletion`, lifecycle은 `pending|claimed|succeeded|retry`이고 `(object_key, storage_etag)`를 unique generation fence로 둔다. authenticated client UPDATE와 DELETE를 허용하지 않는다. server cleanup worker만 Storage API delete authority를 가지며, claim 뒤 profile row/current ref와 Storage owner/key/ETag를 재확인하고 현재 참조 generation은 삭제하지 않는다. #106도 Auth 삭제 전에 같은 server authority로 owner prefix를 정리한다.
