...
- `external_api_snapshots`는 계산 재현 기간 동안 보존한 뒤 `purge_after` 기준의 별도 retention job으로 정리한다. 삭제 시 정규화 내용과 import run lineage는 유지하고 snapshot 포인터만 NULL로 만들며, 새 원문을 연결하기 전에는 내용과 마지막 run을 바꾸지 않는다.
