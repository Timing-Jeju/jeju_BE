...
`RN1`의 `강수없음`은 0 mm, `1mm 미만`은 구간 대표값 0.5 mm로 정규화한다. `30.0~50.0mm`, `50.0mm 이상`처럼 상한이 닫히지 않은 범주는 보수적으로 하한을 저장하며 정확한 문자열은 versioned raw snapshot에 보존한다. 실황 `VEC`는 0도와 360도를 모두 유효한 경계값으로 허용한다.
