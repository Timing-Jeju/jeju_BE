...
- TourAPI 정규화 writer는 `tour_api_operations`의 등록 operation만 허용하고 `tour_api_operation_provenance`에 normalized entity UUID, operation, 선택적 content type, canonical request fingerprint, snapshot과 import run을 기록한다. parameter map은 key 순서와 무관하게 SHA-256으로 만들며 service key·인증값·원문 URL은 제외한다. 정밀 위치 parameter는 원문 컬럼이나 로그 없이 fingerprint에만 반영한다. 한 행의 복수 operation 계보는 보존하고 같은 행/operation/snapshot 중복은 멱등 처리한다.
