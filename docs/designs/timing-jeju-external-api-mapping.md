...
- 외부 정규화 행은 `parsed`/`tombstoned` 원문과 같은 `import_run_id`·`source_snapshot_id`를 반드시 남긴다. 수동·fixture·admin 입력만 명시적 예외이며 이전·새 행이 모두 예외 성격일 때 편집할 수 있다. 외부 lineage 없는 legacy 정규화 행은 source marker를 예외 값으로 바꿔 우회하거나 내용을 변경할 수 없고, marker가 이미 예외 값이어도 OLD snapshot/run의 실제 `source_kind`·provider가 외부이면 내용 변경과 계보 제거를 함께 수행할 수 없다. 유효한 원문과 같은 범위 run을 동시에 붙이는 정상 재수집 repair/upsert로 복구할 수 있으며, 기존 non-NULL lineage와 snapshot-backed optional marker 불일치도 소급 감사한다.
