...
- 마이그레이션은 최초 public 스키마 → `20260730000000` 기본 무결성 → `20260730010000` 외부 적재 기반 → `20260730020000` 적재 일관성 → `20260730030000` 일정 일관성 → `20260730040000` import run 계보 보존 → `20260810000000` 변경 API 멱등성 registry → `20260811000000` async run worker runtime → `20260813000000` import run owner/fencing → `20260813010000` 외부 snapshot 저장 감사 → `20260814000000` TourAPI operation 계보 → `20260816000000` detailInfo2 complete sweep → `20260817000000` detailImage2 complete sweep 계보 → `20260818000000` areaBasedSyncList2 checkpoint seed → `20260819000000` TAGO stop import → `20260823000000` 추천 체류시간 정책 순서로 누적 적용합니다. `20260820000000`~`20260822000000`은 병렬 개발 중인 선행 migration 슬롯으로 예약합니다.
