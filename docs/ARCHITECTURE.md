...
- TAGO 도착정보 adapter는 공개 Controller 없이 application port 뒤에 둡니다. provider/service/city/stop/node 전체 fingerprint의 generation·owner fencing·lease·terminal outcome만 `tago_arrival_flights`에 보존해 20~30초 single-flight가 여러 인스턴스에서도 외부 호출·DB commit을 한 번만 수행합니다. claim/read/terminal CAS는 짧은 SQL로 connection을 즉시 반환하고 외부 callback 동안 DB connection과 transaction을 점유하지 않습니다. leader는 claim 직후 deadline을 다시 검사하고 DB history를 재조회하며, loser는 monotonic deadline 안에서 bounded poll만 하고 deadline 이후 외부 호출을 시작하지 않습니다. expired RUNNING은 즉시 steal하지 않고 `ABANDONED/DATA_UNAVAILABLE` quarantine으로 fail-closed한 뒤 retain 만료 후 새 generation만 허용합니다. 성공과 exact 실패 outcome은 retain window 동안 공유합니다. `RATE_LIMITED`, `TIMEOUT`, `PROVIDER_UNAVAILABLE`일 때만 실패 완료 시각 기준 120초 이하인 마지막 관측을 원래 observed/expires lineage와 stale flag로 반환하고, 공식 `EMPTY_RESULT`·DB `DATA_UNAVAILABLE`·계약 오류에는 stale을 반환하지 않습니다. DB read/mapping 오류는 raw 원인을 숨긴 `DATA_UNAVAILABLE`로 제한해 변환하고 그 밖의 programmer bug는 leader에 그대로 전파하되 follower에는 stable `DATA_UNAVAILABLE`를 공유합니다. exact response bytes의 snapshot terminal 전이, 도착 batch append, run terminal 전이는 한 transaction으로 묶습니다.
