...
- 마이그레이션은 최초 public 스키마부터 timestamp순으로 누적 적용합니다. `20260819000000` TAGO 정류장 적재, `20260820000000` `#36` 노선-정류장 적재, `20260820000001` `#76` KMA 예보, `20260822000000` `#37` 관광지-정류장 후보 link, `20260823000000` `#65` 추천 체류시간 정책, `20260824000000` `#75` TourAPI discovery checkpoint, `20260825000000` `#33` 공개 장소 tombstone, `20260826000000` `#39` TAGO 도착정보 순으로 누적 적용합니다. 병합 전 선택적 선행 migration이 없어도 timestamp 중복을 거부하고 현재 존재하는 canonical migration 전체를 적용합니다.
...
- TAGO 도착정보 adapter는 공개 Controller 없이 application port 뒤에 둡니다. provider/service/city/stop/node 전체를 fingerprint한 PostgreSQL session advisory lock과 인스턴스 내부 future를 함께 사용해 20~30초 single-flight가 여러 인스턴스에서도 외부 호출·DB commit을 한 번만 수행합니다. lock 획득 직후 DB history를 다시 확인하고, loser는 connection을 반납한 뒤 monotonic deadline 안에서만 기다리며 deadline 이후 외부 호출을 중복하지 않습니다. failed future와 advisory lock은 성공·실패 모두 정리합니다. `RATE_LIMITED`, `TIMEOUT`, `PROVIDER_UNAVAILABLE`일 때만 실패 완료 시각 기준 120초 이하인 마지막 관측을 원래 observed/expires lineage와 stale flag로 반환하고, 공식 `EMPTY_RESULT`·DB `DATA_UNAVAILABLE`·계약 오류에는 stale을 반환하지 않습니다. DB read/mapping 오류는 raw 원인을 숨긴 `DATA_UNAVAILABLE`로 제한해 변환하고 그 밖의 programmer bug는 그대로 전파합니다. exact response bytes의 snapshot terminal 전이, 도착 batch append, run terminal 전이는 한 transaction으로 묶습니다.
