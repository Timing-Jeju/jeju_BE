...
- TAGO 도착정보 adapter는 공개 Controller 없이 application port 뒤에 둡니다. provider/service/stop/city/node key별 20~30초 single-flight가 동시에 한 번만 외부 호출·DB commit을 수행하고 failed future를 제거합니다. provider 실패 시 DB의 마지막 관측이 120초 이하일 때만 원래 observed/expires lineage를 가진 stale 결과를 반환하며 normalized write는 추가하지 않습니다. exact response bytes의 snapshot terminal 전이, 도착 batch append, run terminal 전이는 한 transaction으로 묶습니다.
