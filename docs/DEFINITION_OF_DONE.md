...
Issue #112를 상속하는 알림 구현은 Spring이 token·예약·취소·발송·제한 재시도를 단독 소유하고 FastAPI가 관여하지 않는지를 검증해야 합니다. 앱 종료용 메시지는 `notification + data`이며 OS 알림 권한, 서버 출발 알림 설정, 활성 기기를 예약 시점과 발송 직전에 다시 확인하고, 일정 버전 취소·deduplication·generation fencing·짧은 TTL을 통합 테스트로 증명해야 합니다.
