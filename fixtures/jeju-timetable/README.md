...
파일 경계는 `SecureDirectoryStream`의 anchored attribute 조회와 `NOFOLLOW` open을 사용하지만 두 호출이 같은 inode를 원자적으로 보장하지는 않는다. open은 bounded daemon worker에서 `READ+WRITE`로 수행하여 POSIX FIFO open이 block하지 않게 하고, 열린 단일 handle이 seekable인지 확인한 뒤에만 읽는다. worker와 caller 사이의 명시적 handoff가 timeout·interrupt 전에 반환된 handle은 caller에 한 번만 넘기고, 그 뒤 늦게 반환된 handle은 worker가 한 번만 닫는다. 따라서 import 파일은 읽기·쓰기 권한이 모두 필요하며, regular-file 검사 직후 FIFO 교체·재교체나 권한 거부는 별도 pathname writer 없이 fail-closed한다. `SecureDirectoryStream` 미지원 filesystem도 fail-closed한다.
