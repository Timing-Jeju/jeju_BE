...
revoke all on function public.validate_trip_calendar_child() from public;

do $$
begin
  if exists (select 1 from pg_roles where rolname = 'anon') then
    execute 'revoke all on function public.validate_trip_calendar_child() from anon';
    execute 'revoke all on public.trip_transport_events from anon';
  end if;
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    execute 'revoke all on function public.validate_trip_calendar_child() from authenticated';
