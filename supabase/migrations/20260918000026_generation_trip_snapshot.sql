-- #53: command v2 계보를 유지하며 별도의 원문 없는 여행 입력을 불변 보존한다.
begin;

create function timing_jeju_planner_private.generation_input_object(j jsonb, keys text[])
returns boolean language sql immutable security invoker set search_path=''
as $$ select coalesce(jsonb_typeof(j)='object' and j ?& keys and j-keys='{}'::jsonb,false) $$;

create function timing_jeju_planner_private.generation_input_uuid(j jsonb)
returns boolean language sql immutable security invoker set search_path=''
as $$ select coalesce(jsonb_typeof(j)='string' and (j #>> '{}') ~
  '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$',false) $$;

create function timing_jeju_planner_private.generation_trip_input_valid(j jsonb)
...
begin
  ...
  if target_no>day_count or ((target_no=1) is distinct from (j->'baseScheduleVersionId'='null'::jsonb))
...
create table timing_jeju_planner_private.generation_trip_inputs (
  ...
  target_day_id uuid not null,
  base_schedule_version_id uuid,
  schema_version smallint not null default 1 check (schema_version=1),
  structured_input jsonb not null check (octet_length(structured_input::text)<=262144
    and timing_jeju_planner_private.generation_trip_input_valid(structured_input)),
  input_hash text not null check (input_hash ~ '^[0-9a-f]{64}$'),
  created_at timestamptz not null default statement_timestamp(),
  foreign key (run_id,trip_plan_id) references public.itinerary_generation_runs(id,trip_plan_id) on delete cascade,
  foreign key (target_day_id,trip_plan_id) references public.trip_days(id,trip_plan_id) on delete cascade,
  foreign key (base_schedule_version_id,trip_plan_id) references public.trip_schedule_versions(id,trip_plan_id)
);
create index idx_generation_trip_inputs_trip on timing_jeju_planner_private.generation_trip_inputs(trip_plan_id);
create index idx_generation_trip_inputs_owner on timing_jeju_planner_private.generation_trip_inputs(owner_user_id);
create index idx_generation_trip_inputs_day on timing_jeju_planner_private.generation_trip_inputs(target_day_id,trip_plan_id);
create index idx_generation_trip_inputs_base on timing_jeju_planner_private.generation_trip_inputs(base_schedule_version_id,trip_plan_id);

create function timing_jeju_planner_private.guard_generation_trip_input()
returns trigger language plpgsql security invoker set search_path=''
as $$
begin
  if tg_op='UPDATE' then raise exception using errcode='23514',message='generation trip input is immutable'; end if;
  -- 모든 성공 writer와 동일하게 Trip -> run 순서로 잠근다.
  perform 1 from public.trip_plans t where t.id=new.trip_plan_id and t.user_id=new.owner_user_id
    and t.revision=new.trip_revision for share;
  if not found then raise exception using errcode='23514',message='generation trip input lineage mismatch'; end if;
  perform 1 from public.itinerary_generation_runs r
    where r.id=new.run_id and r.trip_plan_id=new.trip_plan_id and r.trip_day_id=new.target_day_id
      and r.requested_by_user_id=new.owner_user_id
      and r.base_schedule_version_id is not distinct from new.base_schedule_version_id
      and r.status='queued' for update;
  if not found then raise exception using errcode='23514',message='generation trip input lineage mismatch'; end if;
  if (new.structured_input->>'tripId')::uuid is distinct from new.trip_plan_id
    or (new.structured_input->>'tripRevision')::bigint is distinct from new.trip_revision
    or (new.structured_input->>'baseScheduleVersionId')::uuid is distinct from new.base_schedule_version_id
    or (new.structured_input->'boundary'->>'dayId')::uuid is distinct from new.target_day_id
    or new.input_hash is distinct from encode(sha256(convert_to(public.canonicalize_command_jsonb(jsonb_build_object(
      'schemaVersion',1,'runId',new.run_id,'ownerId',new.owner_user_id,'tripInput',new.structured_input)),'UTF8')),'hex')
    then raise exception using errcode='23514',message='generation trip input lineage mismatch'; end if;
  return new;
end;
$$;
create trigger trg_generation_trip_input_guard before insert or update
  on timing_jeju_planner_private.generation_trip_inputs for each row
  execute function timing_jeju_planner_private.guard_generation_trip_input();

alter table timing_jeju_planner_private.generation_trip_inputs enable row level security;
revoke all on timing_jeju_planner_private.generation_trip_inputs from public,anon,authenticated,service_role;
grant select,insert on timing_jeju_planner_private.generation_trip_inputs to service_role;
-- invoker guard의 해시 검증에서만 쓰는 순수 canonical 변환기다.
grant execute on function public.canonicalize_command_jsonb(jsonb) to service_role;
revoke all on function timing_jeju_planner_private.generation_input_object(jsonb,text[]),
  timing_jeju_planner_private.generation_input_uuid(jsonb),
  timing_jeju_planner_private.generation_trip_input_valid(jsonb),
  timing_jeju_planner_private.guard_generation_trip_input() from public,anon,authenticated;
grant execute on function timing_jeju_planner_private.generation_input_object(jsonb,text[]),
  timing_jeju_planner_private.generation_input_uuid(jsonb),
  timing_jeju_planner_private.generation_trip_input_valid(jsonb),
  timing_jeju_planner_private.guard_generation_trip_input() to service_role;
