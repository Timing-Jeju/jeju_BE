...
-- Issue #223: every retained user-owned JSON surface is accepted only when
-- its exact reviewed non-location shape and types are known.
...
as $$
  select case input_surface
    ...
    when 'trip_legs.facts' then
      ...
      ) or input_value in ('{"candidate":true}'::jsonb, '{"recovery":true}'::jsonb)
...
as $$
  ...
  select 'trip_items.facts', count(*) from public.trip_items
  where timing_jeju_private.user_json_matches_write_contract(
    'trip_items.facts', facts) is not true
  union all
  select 'trip_legs.facts', count(*) from public.trip_legs
  where timing_jeju_private.user_json_matches_write_contract(
    'trip_legs.facts', facts) is not true
  union all
      ...
      and not exists (
        select 1 from (values
          ...
          ('timing_jeju_planner_private.user_location_residue_counts()',
           '69264bc644b79c06d82bee0a11ded035d70b577dcd21ba8592e470ab2b724979')
