...
alter table public.trip_place_preferences enable row level security;
drop policy if exists trip_place_preferences_owner_select
  on public.trip_place_preferences;

revoke all on function public.validate_trip_place_preference_contract()
  from public, anon, authenticated;
revoke all on function public.validate_trip_place_preference_contract()
  from service_role;
revoke all on function public.validate_trip_place_preference_calendar_change()
  from public, anon, authenticated;
revoke all on function public.validate_trip_place_preference_calendar_change()
  from service_role;
...
revoke truncate, references, trigger on table public.trip_place_preferences from service_role;
