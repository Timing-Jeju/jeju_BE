-- Issue #225: public planned references are the only source of schedule coordinates.
begin;

create schema timing_jeju_planner_private;
revoke all on schema timing_jeju_planner_private from public, anon, authenticated, service_role;
grant usage on schema timing_jeju_planner_private to service_role;

create function timing_jeju_planner_private.resolve_planned_anchor(
  target_kind text, target_anchor_id uuid, target_trip_plan_id uuid
)
returns table (
  anchor_kind text, anchor_id uuid, source_place_id uuid,
  source_stop_id uuid, location public.geography
)
language sql
stable
security invoker
...
as $$
  with place_references as (
    select 'place'::text as kind, place.id as ref_id, place.id as place_id
    from public.tour_places place
    where target_kind = 'place' and place.id = target_anchor_id
      and exists (select 1 from public.trip_plans trip where trip.id = target_trip_plan_id)
    union all
    select 'accommodation', accommodation.id, accommodation.place_id
    from public.trip_accommodations accommodation
    where target_kind = 'accommodation' and accommodation.id = target_anchor_id
      and accommodation.trip_plan_id = target_trip_plan_id
    union all
    select 'transport_event', event.id, event.terminal_place_id
    from public.trip_transport_events event
    where target_kind = 'transport_event' and event.id = target_anchor_id
      and event.trip_plan_id = target_trip_plan_id
  )
  select reference.kind, reference.ref_id, place.id, null::uuid, place.location
  from place_references reference
  join public.tour_places place on place.id = reference.place_id
  where place.location is not null and place.stale = false
    and place.source_deleted_at is null and place.tombstoned_at is null
    and (place.stale_at is null or place.stale_at > now())
  union all
  select 'stop', stop.id, null::uuid, stop.id, stop.location
  from public.bus_stops stop
  where target_kind = 'stop' and stop.id = target_anchor_id and stop.stale = false
    and exists (select 1 from public.trip_plans trip where trip.id = target_trip_plan_id);
$$;

create function timing_jeju_planner_private.resolve_planned_item_anchor(
  target_item_id uuid, target_schedule_version_id uuid, target_trip_plan_id uuid
)
returns table (
  anchor_kind text, anchor_id uuid, source_place_id uuid,
  source_stop_id uuid, location public.geography
...
as $$
  ...
  cross join lateral timing_jeju_planner_private.resolve_planned_anchor(
    case when item.item_type = 'accommodation' then 'accommodation'
         when item.item_type in ('arrival', 'departure') then 'transport_event'
         else 'place' end,
    case when item.item_type = 'accommodation' then item.accommodation_id
         when item.item_type in ('arrival', 'departure') then item.transport_event_id
         else item.place_id end,
    item.trip_plan_id
  ) anchor
  where item.id = target_item_id
    and item.schedule_version_id = target_schedule_version_id
    and item.trip_plan_id = target_trip_plan_id
    and (item.place_id is null or item.place_id = anchor.source_place_id);
$$;

revoke all on function timing_jeju_planner_private.resolve_planned_anchor(text, uuid, uuid)
  from public, anon, authenticated, service_role;
revoke all on function timing_jeju_planner_private.resolve_planned_item_anchor(uuid, uuid, uuid)
  from public, anon, authenticated, service_role;
grant execute on function timing_jeju_planner_private.resolve_planned_anchor(text, uuid, uuid) to service_role;
grant execute on function timing_jeju_planner_private.resolve_planned_item_anchor(uuid, uuid, uuid) to service_role;
