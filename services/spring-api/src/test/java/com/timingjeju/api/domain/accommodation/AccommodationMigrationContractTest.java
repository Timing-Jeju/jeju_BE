...
class AccommodationMigrationContractTest {
  private static final String MIGRATION = "20260906000000_trip_accommodation_contract.sql";
  ...
  void compose와_Docker_upgrade는_036을_seed전에_정확히_적용한다() throws Exception {
    Path root = root();
    String target = "036_trip_accommodation_contract.sql";
    for (String compose :
    ...
    assertThat(Files.readString(root.resolve("scripts/docker-smoke-test.sh")))
        .contains("/docker-entrypoint-initdb.d/036_trip_accommodation_contract.sql");
  }
