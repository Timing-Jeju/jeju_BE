...
public class JdbcAccountDeletionRepository implements AccountDeletionRepository {
  private static final String COLUMNS =
      """
      id, user_profile_id, auth_subject_fingerprint, idempotency_hash, request_hash, status_token_hash,
    ...
    int inserted =
        jdbc.update(
            ...
            insert into public.account_deletion_requests(
              ...
              auth_subject_ciphertext, auth_subject_key_version, status, current_step,
              next_retry_at, requested_at, completed_at)
            values (:id, :profileId, :subjectFingerprint, :idempotencyHash, :requestHash, :tokenHash,
  ...
  public int clearExpiredSecrets(Instant now, Duration terminalRetention, int batchSize) {
    if (terminalRetention == null || terminalRetention.isZero() || terminalRetention.isNegative()) {
      throw new IllegalArgumentException("terminalRetention은 양수여야 합니다.");
    }
    if (batchSize < 1 || batchSize > 1000) {
      throw new IllegalArgumentException("batchSize는 1..1000이어야 합니다.");
    }
    return jdbc.update(
        """
        update public.account_deletion_requests
        set status_token_ciphertext = null, status_token_key_version = null
        where id in (
          ...
          where status_token_ciphertext is not null
            and (status_token_expires_at <= :now
              or (status in ('succeeded', 'failed', 'cancelled')
                and completed_at + (:terminalRetentionSeconds * interval '1 second') <= :now))
          order by status_token_expires_at, id
          limit :batchSize
          for update skip locked
        )
        """,
        new MapSqlParameterSource()
            .addValue("now", utc(now), Types.TIMESTAMP_WITH_TIMEZONE)
            .addValue("terminalRetentionSeconds", terminalRetention.toSeconds(), Types.BIGINT)
            .addValue("batchSize", batchSize, Types.INTEGER));
  }
  ...
  private static MapSqlParameterSource parameters(AccountDeletionRecord record) {
    return new MapSqlParameterSource()
        ...
        .addValue("subjectFingerprint", record.authSubjectFingerprint(), Types.BINARY)
        ...
        .addValue(
            "tokenExpiresAt", utc(record.statusTokenExpiresAt()), Types.TIMESTAMP_WITH_TIMEZONE)
        ...
        .addValue("nextRetryAt", utc(record.nextRetryAt()), Types.TIMESTAMP_WITH_TIMEZONE)
        .addValue("requestedAt", utc(record.requestedAt()), Types.TIMESTAMP_WITH_TIMEZONE)
        .addValue("completedAt", utc(record.completedAt()), Types.TIMESTAMP_WITH_TIMEZONE);
  }
  ...
  private AccountDeletionRecord map(ResultSet rs, int rowNumber) throws SQLException {
    return new AccountDeletionRecord(
        ...
        rs.getBytes("auth_subject_fingerprint"),
