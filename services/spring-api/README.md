# Timing Jeju Spring API

외부 데이터 provider의 환경변수와 시작 검증은 [외부 API 실행 설정](../../docs/EXTERNAL_API_CONFIGURATION.md)을 따릅니다.

Firebase FCM은 기본 비활성이며 ADC/secret mount와 fail-fast 경계는 [Firebase FCM 발송 설정](../../docs/FIREBASE_FCM_CONFIGURATION.md)을 따릅니다.

Timing Jeju의 공개 REST API, 인증·인가, 데이터베이스와 외부 API 연동, MCP 입력 조립과 결과 검증·저장을 담당하는 Spring Boot 서비스입니다.

## 실행

```bash
./gradlew bootRun
```

## 검사

```bash
./gradlew clean check
./gradlew unitTest sliceTest integrationTest architectureTest
```

Docker와 저장소 전체 검증은 저장소 루트에서 `./scripts/quality-gate.sh`를 실행합니다.

### PostgreSQL Repository 통합 테스트

Repository 통합 테스트는 `PostgreSqlRepositoryIntegrationTestSupport`를 상속합니다. 이 공통 기반은 `postgresql-integration` profile과 `@Tag("integration")`을 적용하고, Spring Boot `@ServiceConnection`으로 PostGIS 16 Testcontainer를 임의 포트에 연결합니다. 각 테스트는 Spring 테스트 트랜잭션에서 실행되어 종료 시 자동 rollback됩니다.

컨테이너 초기화에는 `db/local-postgres/auth_compat.sql`을 먼저 사용한 뒤 `supabase/migrations`의 timestamp SQL을 정렬해 원본 그대로 적용합니다. `seed_fixtures.sql`과 `supabase/seed.sql`은 적용하지 않으므로 테스트는 빈 canonical schema에서 시작하며, 필요한 데이터만 테스트 안에서 생성해야 합니다. Flyway, 고정 포트, 컨테이너 재사용 설정, 고정 DB 비밀번호와 별도 환경 변수는 사용하지 않습니다.

```bash
./gradlew integrationTest --tests '*Repository*Test'
```

Docker daemon을 사용할 수 없으면 테스트는 원인을 한국어로 안내합니다. PostgreSQL 17과 실제 Supabase 호환성은 일반 PostgreSQL Testcontainer로 대체하지 않고 저장소 루트의 `./scripts/supabase-smoke-test.sh`에서 두 번의 `supabase db reset`과 함께 검증합니다.

## 외부 데이터 import 실행

`application.importing`의 생명주기 서비스와 port가 import run 시작, count 누적, 성공·부분 성공·실패·취소 전이를 정의하고 `global.importing`의 PostgreSQL adapter가 `data_import_runs`에 원자적으로 반영합니다. provider/service/operation/scope, idempotency key와 요청 fingerprint, parent/retry, parser/schema version을 보존하며, 내부 owner/fencing 값이 일치하지 않는 stale 쓰기를 거부합니다.

이 경계에는 공개 Controller, 실제 외부 API 호출, 개별 importer, raw snapshot 저장, checkpoint 갱신과 scheduler가 포함되지 않습니다. 실패 기록은 고정 분류와 한국어 설명만 사용하고 provider 예외 원문, URL query, key, token과 PII를 저장하거나 출력하지 않습니다.

## 인증·인가

`/api/v1/**`는 Supabase Auth access token이 필요한 stateless Resource Server입니다. 운영과 최신 로컬 CLI는 비대칭 signing key와 JWKS를 사용하며, 기본·운영 issuer/JWKS는 HTTPS만 허용합니다. 정확한 `local` profile은 로컬 JWKS, 정확한 `local-hs256` profile은 legacy HS256만 허용하며 다른 profile과 조합할 수 없습니다. 검증된 현재 사용자는 Spring 비의존 `application.security` 계약으로 제공됩니다. 환경변수와 검증 계약은 저장소의 [인증·인가 설정](../../docs/AUTHENTICATION.md)을 따릅니다.

`GET /api/v1/auth/social/providers`와 `GET /api/v1/auth/social/naver/userinfo`만 Supabase 소셜 OAuth 연동용 공개 endpoint입니다. 카탈로그는 지원 목록이며 Supabase의 실제 활성화 상태가 아닙니다. Google·Kakao는 프론트엔드 Supabase SDK가 시작하고, Naver UserInfo adapter는 provider access token을 저장하지 않으며 엄격한 token/envelope 검증과 rate limit·bulkhead를 적용합니다. Redirect URLs의 단일 권위, Naver PKCE·이메일 신뢰 경계와 `APP_SOCIAL_LOGIN_PROVIDER_IDS`는 [소셜 로그인 설정](../../docs/SOCIAL_LOGIN.md)을 따릅니다.

## FCM 출발 알림 구현 경계

Issue #112의 [FCM 출발 알림 계약](../../docs/contracts/domains/fcm-departure-notification/contract.md)을 후속 구현의 기준으로 사용합니다. Spring이 token registry, 기기 비종속 logical job, per-device delivery attempt, 취소, 발송, 제한 재시도와 provider 접수 상태를 단독 소유하며 FastAPI는 관여하지 않습니다. 앱 종료 상태의 사용자 표시 메시지는 `notification + data`입니다. 복수 기기는 한 logical job, claim commit 후 preparation eligibility 재검사로 고정한 `push_delivery_targets`와 기기별 `(jobId, pushDeviceId, attemptNo)` 단일 attempt row로 fan-out합니다. attempt status는 `RESERVED → CALL_STARTED → terminal` same-row CAS이고 terminal 뒤 immutable합니다. expired `LEASED` reclaim은 state/generation/target/attempt identity를 보존하고 owner/lease/fencing만 새로 발급해 old fence completion을 거부합니다.

generation/expectedGeneration의 single generation naming만 사용합니다. preparation eligible target 0건은 empty snapshot과 `CANCELLED/NO_ACTIVE_PUSH_TARGET`, attempt/provider 0회를 같은 transaction에 기록합니다. target은 `UNATTEMPTED|RETRYABLE → RESERVED → IN_FLIGHT → terminal/retryable`와 `UNATTEMPTED|RETRYABLE → SKIPPED` closed transition만 허용합니다. inactive retry target은 provider 0회로 attemptNo+1 exact terminal `SKIPPED` row·target·job aggregation을 same CAS transaction에 쓰며 duplicate key/stale CAS는 무수정입니다. completion은 existing exact `CALL_STARTED` attempt와 `IN_FLIGHT` target에서만 반영하고 absent/reserved/terminal/wrong-marker/second completion을 거부합니다.

FCM 접수는 단말 전달 완료가 아니다. explicit transient rejection과 request byte 미전송이 증명된 pre-connect failure만 재시도합니다. reclaim 뒤 같은 `RESERVED` row만 retryable failure로, 같은 `CALL_STARTED` row는 post-write/read timeout, connection reset, unexpected EOF와 함께 terminal `ACCEPTANCE_UNKNOWN`으로 바꿉니다. claim↔preparation 기기 변화는 snapshot에 반영하고 snapshot 뒤 신규 기기는 제외하며, 호출 직전 device 비활성은 `SKIPPED`, job-wide 철회는 `CANCELLED`입니다. 세 번째·TTL 만료 transient attempt도 보존한 뒤 job `DEAD`를 원자 반영합니다. OS 알림 권한, 서버 출발 알림 설정, 최신 required 위치 동의를 예약 시점과 발송 직전에 확인하며 invalid consent evidence는 fail-closed합니다. `safetyBufferMinutes` 변경은 preference CAS·old generation/job 취소·재계산을 원자 수행합니다. 앱 재진입 시 푸시 payload 대신 `live-state`를 다시 조회합니다. registration token은 API 응답·로그·trace·metric에 노출하지 않고 Firebase credential은 ADC 또는 secret mount로만 주입합니다. #93과 정정된 #113~#116 구현 및 credential 검증이 완료되기 전에는 production default-off 및 fail-closed입니다.

## 푸시 기기와 알림 설정

`PUT/DELETE /api/v1/me/push-devices/{deviceId}`와 `GET/PATCH /api/v1/me/notification-preferences`는 canonical JWT `sub` 소유 기기와 출발 알림 opt-in을 관리합니다. registration token은 응답과 로그에 포함하지 않고 `PUSH_TOKEN_ENCRYPTION_KEY`로 AES-256-GCM 암호화합니다. 이 변수는 표준 Base64로 인코딩한 정확히 32바이트 키이며 저장소에 실제 운영 값을 두지 않습니다. 상세 계약은 [푸시 기기·출발 알림 설정 계약](../../docs/contracts/domains/push-notifications/contract.md)을 따릅니다.
