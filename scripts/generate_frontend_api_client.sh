...
python3 "${SCRIPT_DIR}/validate_openapi_frontend_readiness.py" "${OPENAPI_PATH}" --mode 21
...
if len(operation_ids) != 21 or missing:
    raise SystemExit(
        f"TypeScript client operation 검증 실패: count={len(operation_ids)}, missing={missing}"
    )
if "tripPlacePreferencesUpdate" not in index:
    raise SystemExit("장소 선호 client operation이 없습니다: tripPlacePreferencesUpdate")
