...
let outputPath = CommandLine.arguments.count > 1
  ...
  : "docs/proposals/timing-jeju-4page-12pt-prose-proposal.pdf"
...
let lightBlue = NSColor(calibratedRed: 0.93, green: 0.97, blue: 1.0, alpha: 1)
...
final class Renderer {
  ...
  func endPage() {
    drawRule(top: 802)
    drawText("타이밍제주 | 12pt 이상 · 4페이지 본문형 작성본", x: margin, top: 811, width: contentWidth - 70, font: bodyFont, color: NSColor.darkGray, lineHeight: 13)
  ...
  @discardableResult
  func callout(_ text: String, top: CGFloat, height: CGFloat = 42) -> CGFloat {
    ctx.saveGState()
    ...
    ctx.restoreGState()
    _ = drawText(text, x: margin + 10, top: top + 8, width: contentWidth - 20, font: bodyBold, color: blue, align: .center, lineHeight: 14.5, maxHeight: height - 10)
    return top + height + 8
  }
...
y = r.drawText("제주도는 국내 대표 관광지이지만 렌터카 없이 이동하는 여행자에게는 일정 운영 난도가 높은 지역이다. 관광지가 동서남북으로 넓게 분산되어 있고, 관광지와 정류장 사이의 도보 이동, 버스 배차 간격, 환승 가능 여부가 하루 일정의 성패를 좌우한다. 기존 지도 앱은 A에서 B까지의 경로를 알려주지만, 여행자가 하루 전체 일정을 기준으로 언제 나와야 하는지, 더 머물러도 되는지, 버스를 놓쳤을 때 어떤 일정을 조정해야 하는지는 판단하기 어렵다.", x: margin, top: y, width: contentWidth)
y = r.drawText("제주 관광 수요는 충분하다. 2024년 제주 방문 관광객은 약 1,378만 명 규모이며, 제주관광공사 비짓제주는 2024년 1월 1일부터 11월 30일까지 누적 방문자 531만 6,274명을 기록했다. 온라인 관광정보 탐색 수요는 이미 크지만, 렌터카 없이 이동하는 관광객이 버스 시간까지 반영해 하루 일정을 운영하도록 돕는 서비스는 부족하다.", x: margin, top: y, width: contentWidth)
y = r.callout("핵심 문제는 ‘어디를 갈지’가 아니라 ‘버스 시간에 맞춰 그 일정을 실제로 실행할 수 있는지’이다.", top: y)
...
y = r.callout("타이밍제주는 TourAPI 관광정보와 제주 버스 데이터를 결합해 렌터카 없이 제주를 여행하는 뚜벅이의 일정 실패를 줄이는 제주 특화 웹·앱 서비스이다.", top: y, height: 48)
...
y = r.drawText("이 기능들은 공모전 기능심사에서도 하나의 흐름으로 시연할 수 있다. 사용자가 제주 동쪽 하루 일정을 입력하면 서비스가 TourAPI로 장소를 확인하고, 버스 시간표로 출발 시각을 계산하며, 관광 중 지연이 발생했을 때 주변 대체지와 재일정안을 제시한다. 즉 아이디어 설명에 그치지 않고 실제 동선 기반 프로토타입으로 보여줄 수 있다.", x: margin, top: y, width: contentWidth)
r.warnIfOverflow(y)
r.endPage()
...
y = r.drawText("서비스의 초기 범위를 제주 동쪽으로 좁히는 것도 완성도를 높이기 위한 선택이다. 공모전 개발 기간에는 모든 지역을 얕게 다루기보다, 실제 심사에서 작동하는 핵심 경험을 만드는 것이 중요하다. 동쪽 코리도어에서 장소 매칭, 정류장 탐색, 출발 권장 시각 계산, 일정 안전도, 남는 시간 추천, 재일정까지 완성하면 이후 제주 전역으로 확장할 수 있는 구조도 함께 검증된다.", x: margin, top: y, width: contentWidth)
r.warnIfOverflow(y)
r.endPage()
...
y = r.drawText("2단계는 제주 전역 확장이다. MVP에서 검증한 데이터 구조를 바탕으로 제주 전역의 정류장, 노선, 시간표 데이터를 확대 적용한다. 3단계는 실시간 보정과 다국어 안내다. 버스 위치 또는 도착 예정 정보가 확보되면 안내 정확도를 높이고, 외국인 관광객을 위해 한국어 일정 안내를 영어·중국어·일본어로 제공한다. 4단계는 RTO 대시보드다. 대기 위험 구간과 이동 불편 구간을 분석해 지역관광 정책과 관광객 분산 방문에 활용할 수 있다.", x: margin, top: y, width: contentWidth)
...
y = r.drawText("기술 구현은 백엔드 Spring Boot, 프론트엔드 React 기반 프레임워크, OpenAI API, TourAPI, 제주 버스 데이터 수집·정규화 구조로 진행한다. 핵심 계산은 서버에서 수행하고 프론트엔드는 일정 타임라인, 안전도 상태, 추천 카드, 재일정 결과를 보여준다. 이 구조는 공모전 기간 내 MVP 구현 가능성이 높고, 향후 모바일 앱이나 알림 서비스로 확장하기도 쉽다.", x: margin, top: y, width: contentWidth)
r.warnIfOverflow(y)
r.endPage()
