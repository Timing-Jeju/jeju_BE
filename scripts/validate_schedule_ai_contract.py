...
IDENTITIES = [
    ...
    ("POST", "/api/v1/trips/{tripId}/schedule-generations/{runId}/candidates/{candidateId}/apply", "apply", [200], [400, 401, 404, 409, 410, 422, 429, 503]),
...
TOP_FIELDS = {
    ...
    "securityPolicy", "locationInputPolicy", "generationSnapshotPolicy", "generationCandidateSetPolicy",
    "generationPersistencePolicy", "databasePolicy", "retentionPolicy", "idempotencyPolicy",
...
def resolve_apply_overlap(state: dict[str, Any]) -> str:
    ...
    rules = [
        ...
        (
            not state.get("liveCandidatePayloadAvailable", True),
            "CANDIDATE_EVIDENCE_UNAVAILABLE",
        ),
        (not state.get("tripEtagMatches", True), "TRIP_VERSION_CONFLICT"),
        (
...
def validate(contract_path: Path, catalog_path: Path = CATALOG) -> list[str]:
    ...
    if not isinstance(schemas, dict) or set(schemas) != expected_schemas:
        ...
        for name in ["AsyncCreateHeaders", "GenerationCreateHeaders", "AsyncApplyHeaders"]:
    ...
    generation_request = schemas.get("GenerationRunRequest", {}) if isinstance(schemas, dict) else {}
    expected_generation_request = {
        "type": "object",
        "additionalProperties": False,
        "required": ["targetDayId", "expectedActiveScheduleVersionId", "candidateCount"],
        "properties": {
            "targetDayId": {"type": "string", "format": "uuid", "nullable": False},
            "expectedActiveScheduleVersionId": {
                "type": "string",
                "format": "uuid",
                "nullable": True,
            },
            "candidateCount": {"type": "integer", "const": 3, "nullable": False},
        },
    }
    if generation_request != expected_generation_request:
        errors.append("generation 접수는 target day, nullable expected active와 정확히 3개 후보만 허용해야 합니다.")

    generation_result = schemas.get("GenerationResult", {}) if isinstance(schemas, dict) else {}
    result_properties = generation_result.get("properties", {})
    candidates_schema = result_properties.get("candidates", {})
    if (
        "outcome" not in generation_result.get("required", [])
        or result_properties.get("outcome", {}).get("enum")
        != ["success", "insufficient_feasible_routes"]
        or result_properties.get("baseScheduleVersionId", {}).get("nullable") is not True
        or candidates_schema.get("minItems") != 0
        or candidates_schema.get("maxItems") != 3
    ):
        errors.append("generation result의 outcome, nullable base, 0/3 candidate 경계가 정확하지 않습니다.")

    generation_candidate = schemas.get("GenerationCandidate", {}) if isinstance(schemas, dict) else {}
    candidate_properties = generation_candidate.get("properties", {})
    if (
        candidate_properties.get("strategy", {}).get("enum")
        != ["balanced", "relaxed", "experience_max"]
        or not {"strategy", "feasibility", "scheduleUrl", "applyUrl"}.issubset(
            generation_candidate.get("required", [])
        )
    ):
        errors.append("generation candidate의 전략·가능성·조회·적용 link가 정확하지 않습니다.")

    apply_request = schemas.get("ApplyCandidateRequest", {}) if isinstance(schemas, dict) else {}
    if apply_request.get("properties", {}).get("expectedActiveScheduleVersionId", {}).get(
        "nullable"
    ) is not True:
        errors.append("최초 candidate 적용은 nullable expected active schedule version을 허용해야 합니다.")

    generation_endpoints = contract.get("endpoints", [])
    generation_create = generation_endpoints[0] if generation_endpoints else {}
    if (
        generation_create.get("schemas", {}).get("headers") != "GenerationCreateHeaders"
        or generation_create.get("headers") != ["Authorization", "Idempotency-Key", "If-Match"]
    ):
        errors.append("generation 접수는 최신 Trip strong If-Match를 요구해야 합니다.")

    candidate_set_policy = contract.get("generationCandidateSetPolicy")
    expected_candidate_set_policy = {
        "success": {
            "candidateCount": 3,
            "strategiesExactlyOnce": ["balanced", "relaxed", "experience_max"],
    ...
    if candidate_set_policy != expected_candidate_set_policy:
        errors.append("generation 결과는 세 전략 성공 또는 후보 0개 실패로 원자적이어야 합니다.")

    persistence_policy = contract.get("generationPersistencePolicy")
    if not isinstance(persistence_policy, dict) or persistence_policy.get("neverPersistOrLog") != [
        "tmapRawResponse",
        "detailedGeometry",
        "userOriginalText",
    ] or persistence_policy.get("tmapProcessMemoryMaximumTtl") != "PT23H50M" or persistence_policy.get(
        "persistProviderDerivedRouteValues"
    ) is not True:
        errors.append("TMAP 원본·geometry·사용자 원문 비영속 계약이 정확하지 않습니다.")
    if isinstance(persistence_policy, dict) and (
        persistence_policy.get("candidatePayloadStorage") != "approved_normalized_durable_projection"
        or persistence_policy.get("candidatePayloadMaximumTtl") != "PT24H"
        or persistence_policy.get("onProcessRestartOrPayloadLoss")
        != "restore_validated_projection_or_candidate_evidence_unavailable"
        or persistence_policy.get("terminalRunRetentionAfterCandidatePayloadExpiry")
        != "metadata_only"
    ):
        errors.append("승인된 정규화 후보의 durable TTL과 복원 계약이 정확하지 않습니다.")
    retention = contract.get("retentionPolicy", {})
    if retention.get("candidate") != "PT24H after candidate createdAt; normalized durable snapshot only":
        errors.append("정규화 후보는 생성 시각부터 24시간만 보존해야 합니다.")
