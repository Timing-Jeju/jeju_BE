...
class FrontendApiClientGenerationTest(unittest.TestCase):
    def test_generator는_검증된_29_operation과_고정_codegen_version만_사용한다(self):
        ...
        self.assertIn('validate_openapi_frontend_readiness.py" "${OPENAPI_PATH}" --mode 29', script)
    ...
    def test_network_free_verifier는_29_operation_artifact와_index를_검증한다(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                ["python3", str(verifier), str(openapi), str(output), "29"],
                cwd=ROOT,
                capture_output=True,
                text=True,
            )
        ...
        self.assertIn("29 operations", result.stdout)
