...
class Issue114DevelopIntegrationTest(unittest.TestCase):
    ...
    def test_firebase_adapter와_현재_mode25_historical_mode24가_함께_유지된다(self):
        firebase_adapter = (
            ROOT
        ...
        for gate_name in ("quality-gate.sh", "quality-gate.ps1"):
            ...
            self.assertIn("--mode 25", gate, gate_name)
        ...
        self.assertIn("choices=(9, 16, 20, 21, 23, 24, 25)", validator)
