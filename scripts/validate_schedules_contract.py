...
def validate(contract_path: Path = DEFAULT_CONTRACT, skip_catalog_fixtures: bool = False, catalog_path: Path = CATALOG) -> list[str]:
    ...
    if item_policy.get("emptyDay") != "reject-422 SCHEDULE_DAY_EMPTY; DELETE or cross-Day move must leave at least one item in every trip Day":
        errors.append("빈 Day 금지와 SCHEDULE_DAY_EMPTY 계약이 필요합니다.")
