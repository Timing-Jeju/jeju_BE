...
이 값을 늘리면 `docker stop --time`(240초)과 systemd `TimeoutStopSec`(270초)도 함께 늘려 강제 종료보다 짧게 유지한다.
...
컨테이너에서 메타데이터 서버(169.254.169.254)로 가는 TCP/UDP는 DNS(53)를 제외하고 차단한다.
GCE에서는 같은 주소가 VPC DNS resolver이므로, 53까지 막으면 DB·JWKS 등 모든 호스트 이름 조회가 실패한다.
애플리케이션은 비특권 사용자와 읽기 전용 파일시스템으로 실행하며 Docker 요청/본문 로그를 남기지 않는다.
배포 참조값(이미지 digest, secret 버전, `secret_files`, `private_host_mappings`)은 시작 스크립트가 아니라
인스턴스 메타데이터 `timing-jeju-config`에 있고, 서비스가 시작될 때마다 다시 읽는다. 비밀값은 포함하지 않는다.
...
배포 참조값을 바꾼 apply는 VM을 교체하지 않고 메타데이터만 수정한다. 실행 중인 컨테이너는 그대로 유지되며,
새 값은 BE 서비스가 다음에 시작될 때 적용된다.

1. 새 digest/secret version으로 plan을 만들고 `google_compute_instance.be`가 **update in-place**인지 확인한다.
   replace가 보이면 apply하지 않고 원인을 검토한다.
2. 신규 생성 접수 중단과 진행 작업 drain을 확인한 뒤 apply한다.
3. 승인된 IAP 접근으로 `sudo systemctl restart timing-jeju-be`를 실행한다. Spring 정상 종료를 기다린 뒤
   새 설정으로 secret을 조회하고 이미지를 받아 시작한다. 준비 단계가 실패하면 컨테이너는 시작되지 않는다.
4. health와 로그인·일정 생성 등 주요 기능을 확인한다. 롤백은 이전 digest/version으로 같은 절차를 반복한다.

apply 이후에는 장애 재시작, Docker 재시작, VM 재부팅 등 **어떤 이유로든 서비스가 다시 시작되면 새 값이 적용된다.**
배포 시점이 아니면 apply하지 않는다. 3단계에는 `enable_iap_ssh=true`와 운영자 IAP/OS Login 권한이 필요하다.
시작 스크립트(`metadata_startup_script`) 자체를 바꾸면 VM 교체가 필요하며 `deletion_protection` 때문에 apply가 실패한다.
스크립트 변경은 이미지 교체와 분리해 별도 교체 계획으로 다룬다.
...
Spring 정상 종료 단계 제한 시간은 210초, Docker 종료 대기는 240초, systemd 중지 제한은 270초로 둔다.
...
5. 복구되지 않으면 [이미지·비밀 버전 교체와 롤백](#이미지비밀-버전-교체와-롤백) 절차로 승인된 이전 이미지/비밀 버전을 복원한다.
